...
function renderTable(tasks) {
  ...
  tbody.innerHTML = '';
  // Normalise indent so the shallowest outline level always starts at 0.
  // Using allTasks (not the filtered list) keeps indents stable while filtering.
  const minOutline = state.allTasks.length
    ? Math.min(...state.allTasks.map(t => t.outline))
    : 0;
  for (const task of tasks) {
    const indent      = Math.max(0, task.outline - minOutline) * 16;
    ...
    tr.innerHTML =
      ...
      `<td class="col-name"><span class="task-name" style="padding-left:${indent}px">${chevron}<span class="task-text">${escapeHtml(task.name)}</span></span></td>` +
